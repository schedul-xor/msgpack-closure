...
function getDependencies(config, exports, callback) {
  if(callback === undefined){
    callback(new Error('callback is undefined'));
    return;
  }
  writeExports(exports, function(err, exportsPath) {
    ...
    log.info('msgpack', 'Parsing dependencies');
    var options;
    if (config.src) {
      options = {
        lib: config.src,
...
function build(config, paths, callback) {
  ...
  if (!options.compile) {
    log.info('msgpack', 'No compile options found.  Concatenating ' +
        paths.length + ' sources');
    concatenate(paths, callback);
  } else {
    log.info('msgpack', 'Compiling ' + paths.length + ' sources');
